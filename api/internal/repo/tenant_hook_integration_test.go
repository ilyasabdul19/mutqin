...
func TestTenantHook_NarrowsUserGetByID(t *testing.T) {
	...
	userRepo := repo.NewUserRepo(testAdminHooked)
...
func TestTenantHook_OrganizationLookupUnaffected(t *testing.T) {
	...
	gotApp, err := repo.NewOrganizationRepo(testAdminHooked).GetBySlugAdmin(ctx, "z")
	if err != nil {
		t.Fatalf("hooked GetBySlugAdmin (no tenant ctx) failed: %v", err)
	}
	_ = gotApp

	// Place a tenant in ctx and query the organizations table (which has no
	// organization_id column). The hook MUST detect the missing column and
	// skip injection — otherwise Postgres errors with `column "organization_id"
	// does not exist`.
	uniqueOrg := uuid.New()
	ctxScoped := tenant.With(ctx, uniqueOrg)
	if _, err := repo.NewOrganizationRepo(testAdminHooked).GetBySlugAdmin(ctxScoped, "z"); err != nil {
